...
- 已有桌宠客户端第一版：桌宠属于“卡咔 IP”部分，是卡咔的桌面身体、状态表现、互动手感和素材承载层；不是“卡咔机器人”核心本身。前端和 Tauri 工程在 `apps/desktop-pet`，技术组合为 Tauri 2 + React/Vite + TypeScript + PixiJS。当前是透明、无边框、置顶且不占任务栏的桌面窗口，已接入 12 张桌宠透明 PNG 状态图，支持右键状态菜单、退出、轻点摸头、拖拽窗口、拖拽反应恢复、闲置睡觉、睡眠唤醒、窗口位置记忆、随机待机气泡/小动作、`kaka-core /health` 连接测试和消息气泡雏形。已新增系统托盘入口：托盘图标、显示/隐藏、设置、开机自启、重置位置、修复窗口、连接测试、重启桌宠和退出。设置窗口支持查看开机自启和核心连接状态，执行开机自启、连接测试、重置位置，配置随机待机、闲置睡觉时间、“开机自启时显示卡咔”、桌宠大小、气泡停留时间、随机待机频率、始终置顶、顶部状态标签和右键调试状态菜单；桌宠大小会同步调整主窗口尺寸和 Pixi 本体渲染比例。自启注册会带 `--from-autostart` 参数，默认开机自启时只驻留托盘；手动启动仍显示卡咔。托盘菜单状态已优化：主窗口显示时菜单显示“隐藏卡咔”，隐藏时显示“显示卡咔”；设置窗口读取或切换开机自启后会同步刷新托盘勾选状态。设置窗口已补细节打磨：状态颜色标签、操作中按钮文案、控件禁用态、成功/失败/检查中提示样式。设置窗口已补主窗口控制：可查看桌宠窗口显示/隐藏状态，并在设置里显示/隐藏卡咔或退出程序。右键状态菜单已收敛：默认显示说话、摸摸头、设置、隐藏、退出；设置里打开“启用右键调试状态菜单”后才显示连接测试、重置位置和状态切换。设置窗口已拆成独立 `settings.html` 入口，并由 Tauri 启动时预创建、默认隐藏；打开设置只 show/focus，右上角关闭按钮只隐藏窗口，避免动态创建 WebView 导致白屏卡死。摸头、待机、睡醒和拖拽松手已有轻量随机气泡/状态反馈；已新增 `petBehavior.ts` 本地行为系统，统一管理摸头、双击、长按、拖拽过久、待机、睡醒和睡觉反应，支持权重、防重复、冷却和连续摸头计数。桌宠已完成轻量输入入口和最小 `kaka-core /v1/chat` 接入：右键“说话”打开小输入框，发送后由 Tauri Rust 后端组装 `platform=desktop` 的 `MessageEvent` 请求 `/v1/chat`，取 `KakaResponse.actions` 第一条 `send_text` 文本显示到气泡；失败时切到信号弱并保留输入。对话展示层已完成第一阶段打磨：长回复会拆成连续气泡，气泡时长随文本长度延长，回复内容会本地推断 `思考` / `开心` / `收到消息` 状态，失败提示会区分连接失败、超时和响应异常。对话入口也已补日常手感：双击卡咔可打开说话，失败后按钮显示 `重试`，中文输入法 Enter 不会误发送，输入框内保留本次运行最近 6 条本地临时对话。第二阶段轻量打磨已补：短普通确认不会频繁切开心，输入框相关提示改到面板内部，最近对话默认折叠且可展开完整查看。当前还没有完整聊天历史窗口、快捷唤醒或语音入口。
...
- 桌宠 v0.1 收口检查已完成：已复查 `apps/desktop-pet` 前端主窗口、设置窗口、Pixi 渲染、行为/状态、聊天展示和 Tauri 托盘/命令代码，当前没有发现必须修复的新问题；在现有 12 张静态 PNG 素材范围内，桌宠本体功能和互动基本收口。下一阶段明显提升需要新素材或新动画方案。已新增 `卡咔桌宠/卡咔桌宠当前状态与后续计划.md`，专门记录卡咔 IP 桌宠线，避免和卡咔机器人核心、QQ、记忆、Web 管理台文档混写。
...
2026-05-28 desktop-pet 启动体验优化：`npm run build` passed；`cargo check` passed
2026-05-28 desktop-pet 托盘菜单状态优化：`npm run build` passed；`cargo check` passed；`cargo test` passed；`git diff --check` passed（仅 CRLF 提示）；`npm run tauri:build` passed；用户实测无问题
...
2026-05-30 desktop-pet v0.1 收口检查：已复查主窗口、设置窗口、Pixi 渲染、行为/状态、聊天展示和 Tauri 托盘/命令代码，未发现必须修复的新问题；`npm run build` passed；`cargo check` passed；`cargo test` passed（3 passed）；`git diff --check` passed；`npm run tauri:build` passed；release exe 已重新启动，当前 PID 60544；`cargo fmt --check` 和 `cargo clippy` 因本机缺少 `rustfmt` / `clippy` 组件未执行
