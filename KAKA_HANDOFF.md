...
- 已有桌宠客户端第一版：前端和 Tauri 工程在 `apps/desktop-pet`，技术组合为 Tauri 2 + React/Vite + TypeScript + PixiJS。当前是透明、无边框、置顶且不占任务栏的 `280x280` 桌面窗口，已接入 12 张桌宠透明 PNG 状态图，支持右键状态菜单、退出、轻点摸头、拖拽窗口、拖拽反应恢复、2 分钟闲置睡觉、睡眠唤醒、窗口位置记忆、随机待机气泡/小动作、`kaka-core /health` 连接测试和消息气泡雏形。已新增系统托盘基础入口：托盘图标、显示/隐藏、开机自启、重置位置、连接测试和退出。桌宠当前还没有接入 `kaka-core /v1/chat` 对话。
