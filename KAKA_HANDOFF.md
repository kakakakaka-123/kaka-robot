...
- 已有桌宠客户端第一版：前端和 Tauri 工程在 `apps/desktop-pet`，技术组合为 Tauri 2 + React/Vite + TypeScript + PixiJS。当前是透明、无边框、置顶且不占任务栏的 `280x280` 桌面窗口，已接入 12 张桌宠透明 PNG 状态图，支持右键状态菜单、退出、轻点摸头、拖拽窗口、拖拽反应恢复、闲置睡觉、睡眠唤醒、窗口位置记忆、随机待机气泡/小动作、`kaka-core /health` 连接测试和消息气泡雏形。已新增系统托盘基础入口：托盘图标、显示/隐藏、设置、开机自启、重置位置、连接测试和退出。已新增设置窗口基础版：查看开机自启和核心连接状态，执行开机自启、连接测试、重置位置，配置随机待机、闲置睡觉时间和“开机自启时显示卡咔”。自启注册会带 `--from-autostart` 参数，默认开机自启时只驻留托盘；手动启动仍显示卡咔。托盘菜单状态已优化：主窗口显示时菜单显示“隐藏卡咔”，隐藏时显示“显示卡咔”；设置窗口读取或切换开机自启后会同步刷新托盘勾选状态。设置窗口已补细节打磨：状态颜色标签、操作中按钮文案、控件禁用态、成功/失败/检查中提示样式。设置窗口已补主窗口控制：可查看桌宠窗口显示/隐藏状态，并在设置里显示/隐藏卡咔或退出程序。桌宠当前还没有接入 `kaka-core /v1/chat` 对话。
