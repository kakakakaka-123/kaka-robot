...
- 已有桌宠客户端第一版：前端和 Tauri 工程在 `apps/desktop-pet`，技术组合为 Tauri 2 + React/Vite + TypeScript + PixiJS。当前是透明、无边框、置顶且不占任务栏的 `280x280` 桌面窗口，已接入 12 张桌宠透明 PNG 状态图，支持右键状态菜单、退出、轻点摸头、拖拽窗口、拖拽反应恢复、闲置睡觉、睡眠唤醒、窗口位置记忆、随机待机气泡/小动作、`kaka-core /health` 连接测试和消息气泡雏形。已新增系统托盘入口：托盘图标、显示/隐藏、设置、开机自启、重置位置、修复窗口、连接测试、重启桌宠和退出。已新增设置窗口基础版：查看开机自启和核心连接状态，执行开机自启、连接测试、重置位置，配置随机待机、闲置睡觉时间和“开机自启时显示卡咔”。自启注册会带 `--from-autostart` 参数，默认开机自启时只驻留托盘；手动启动仍显示卡咔。托盘菜单状态已优化：主窗口显示时菜单显示“隐藏卡咔”，隐藏时显示“显示卡咔”；设置窗口读取或切换开机自启后会同步刷新托盘勾选状态。设置窗口已补细节打磨：状态颜色标签、操作中按钮文案、控件禁用态、成功/失败/检查中提示样式。设置窗口已补主窗口控制：可查看桌宠窗口显示/隐藏状态，并在设置里显示/隐藏卡咔或退出程序。右键状态菜单已收敛：默认只显示摸摸头、设置、隐藏、退出；设置里打开“启用右键调试状态菜单”后才显示连接测试、重置位置和状态切换。设置窗口已拆成独立 `settings.html` 入口，并由 Tauri 启动时预创建、默认隐藏；打开设置只 show/focus，右上角关闭按钮只隐藏窗口，避免动态创建 WebView 导致白屏卡死。摸头、待机、睡醒和拖拽松手已有轻量随机气泡/状态反馈。桌宠当前还没有接入 `kaka-core /v1/chat` 对话。
