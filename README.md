...
- 基础人设 Prompt 文件：`prompts/kaka_persona.md` 会作为回复时 System Prompt 的基础层；关系上下文和长期记忆会在它之后动态追加。路径可用 `KAKA_PERSONA_PROMPT_PATH` 调整，文件缺失时会回退到内置基础人设。2026-05-23 已更新为第一版电子猫娘运行 Prompt：短回复、猫感、轻微电波、嘴硬心软、创造者大人特殊关系和记忆使用边界；2026-05-24 已用真实 SQLite 测试输入和真实 DeepSeek 调用做小样本人设回放，并收紧摸头权限梗、被叫 AI、负面情绪缓冲、技术回答去客服化和记忆缺失边界。
...
- 2026-05-19 已按当时要求回退强化人设实验：删除三层人格定义和人设评测脚本，清理评测数据；2026-05-23 在保留干净工程地基的前提下，重新把用户认可的长期人设底稿、场景样例和负面情绪缓冲规则纳入文档，并更新运行 Prompt；2026-05-24 已完成真实 LLM 小样本人设回放，测试输入均已标记 `skipped`，不进入长期记忆候选分析。
...
- 程序内置自动候选区复核：可选开启后，`kaka-core` 在下一个整点及之后每个整点检查一次；`pending >= 20` 时最多处理一批，每批 10 条；每次检查会写入 `auto_job_runs`。运行状态页可手动触发一次，手动触发会绕过数量门槛。
- 自动任务锁：自动候选分析和自动候选区复核都有跨进程任务锁，避免多个仍在运行的 `kaka-core` 进程在整点或手动触发时重复执行。
...
memories：已经从候选区合并出的正式长期记忆，后续回复前检索只应读取 active 记忆
...
当前已经完成“回复时读取正式长期记忆”的第一版接入：`search_memories.py` 的检索逻辑已经抽成正式模块，`kaka-core` 回复前会按当前用户、场景和消息检索少量 `active` 记忆，并把命中的记忆作为可参考背景放进模型 prompt。当前也已接入第一版短期上下文，默认读取同场景最近 30 分钟内最多 20 条输入记录，总字符上限 1200，排除当前消息。回复前还会注入第一版关系上下文，按主人、熟人、普通熟悉群友和陌生人粗分表达边界。基础人设已经从代码拆到 `prompts/kaka_persona.md`，当前运行版强调电子猫娘、短回复、猫感、电波感、嘴硬心软和创造者大人特殊关系；更完整的人设方向、场景样例和负面情绪缓冲规则分别沉淀在 `docs/卡咔人设设定.md`、`docs/卡咔场景反应样例.md` 和 `docs/卡咔负面情绪处理规则.md`。2026-05-24 已用真实 DeepSeek 回放过摸头、被叫 AI、负面情绪、技术问题和记忆缺失等场景，并据此微调运行 Prompt。回复上下文内部已经分为 `persona / relationship / memory / recent_context / current_message` 层，方便后续继续接入用户画像和情绪状态。长期记忆、短期上下文、关系阈值和人设路径都可通过 `.env` 调整。程序里也已经内置自动候选分析和自动候选区复核，两者都在整点触发；运行状态页可以手动触发一次，跨进程任务锁会避免重复执行。
...
当前暂时不要做图片、表情包、语音和复杂主动行为。下一步优先做真实 QQ 小范围人设测试，先保证文本闭环、原始记录、开发工具和卡咔味道长期稳定。
