...
- `apps/desktop-pet`：卡咔桌宠第一版原型。技术组合为 Tauri 2 + React/Vite + TypeScript + PixiJS；当前是透明、无边框、置顶的 280x280 桌面窗口，已接入 12 张透明 PNG 状态图，支持右键状态菜单、轻点摸头反应、拖拽窗口、拖拽反应恢复、2 分钟闲置睡觉、睡眠唤醒和消息气泡雏形。桌宠当前还没有接入 `kaka-core`，先作为本地互动身体原型独立运行。
...
kaka-v2/
  apps/
    ...
    desktop-pet/         # 桌宠客户端，Tauri + React/Vite + PixiJS
    iot-adapter/         # 后期硬件接入
...
2026-05-27 已启动桌宠方向。当前桌宠在 `apps/desktop-pet`，定位是卡咔的桌面身体原型，不替代 `kaka-core`。第一版已完成透明置顶窗口、PixiJS 贴图渲染、12 个状态、右键菜单、摸头、拖拽、闲置睡觉和消息气泡。下一步应把气泡输入/输出接入 `kaka-core`，让桌宠能发送 `platform=desktop` 的 `MessageEvent` 并显示核心大脑回复。
