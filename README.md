...
2026-05-27 已启动桌宠方向。当前桌宠在 `apps/desktop-pet`，定位是卡咔的桌面身体原型，不替代 `kaka-core`。第一版已完成透明置顶窗口、PixiJS 贴图渲染、12 个状态、右键菜单、摸头、拖拽、闲置睡觉、消息气泡、托盘设置和最小核心对话接入。桌宠现在可以发送 `platform=desktop` 的 `MessageEvent` 并显示核心大脑回复；下一步桌宠线的重点不再是“能否接入 core”，而是素材质量、动画方案、完整聊天历史窗口和更自然的桌面互动。
