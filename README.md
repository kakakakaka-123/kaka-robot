...
当前已经完成“回复时读取正式长期记忆”的第一版接入：`search_memories.py` 的检索逻辑已经抽成正式模块，`kaka-core` 回复前会按当前用户、场景和消息检索少量 `active` 记忆，并把命中的记忆作为可参考背景放进模型 prompt。当前也已接入第一版短期上下文，默认读取同场景最近 30 分钟内最多 8 条输入记录，总字符上限 1200，排除当前消息。长期记忆和短期上下文都可通过 `.env` 开关控制。程序里也已经内置自动候选分析和自动候选区复核，两者都在整点触发。
...
4. 响应 metadata 会记录 `used_memory_ids`、`memory_count`、`memory_injection_enabled`、`short_context_input_ids` 和 `short_context_count`。
5. 后续情绪、关系和人设扩展应继续接入 `kaka_core.context.builder`，不要散落在聊天服务里。
