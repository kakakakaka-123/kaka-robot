...
def test_growth_candidates_sorted_by_stars_for_stable_snapshots() -> None:
    # 增长候选必须按 stars 排序：sort=updated 每周返回的"最近推送"集合会整体翻新，
    # 与上周快照几乎不重叠，导致真实增长恒为 0、增长榜首跑后长期为空。按 stars
    # 排序让被追踪的候选集合周与周之间稳定，快照才能命中并算出真实增长。
    code = all_code(load_workflow())
    growth_blocks = [
        block
        for block in code.split("section:")
        if block.lstrip().startswith("'growth'")
    ]
    assert growth_blocks, "expected a growth query section in the workflow"
    for block in growth_blocks:
        head = block[: block.find("per_page")]
        assert "sort: 'stars'" in head
        assert "sort: 'updated'" not in head
